...
class JParser{		
	...
	public $parsed;
	public $generate_tags = '';
	
	// Function to parse BBCode
	public function parse2html($input = '', $custom_tags = ''){
		if($custom_tags == '' && $input != ''){
			$this->parsed = strip_tags($input);
		}else{
			$this->generate_tags = $custom_tags;
		}
		if($this->generate_tags != '' && $input != ''){
			$this->parsed = $input;
			foreach($this->generate_tags as $tag => $val){
				$this->parsed = str_ireplace($tag, $val, $this->parsed);
			}
		}
	...
	//Function to add custom BBCode Tags
	public function add_tags($custom_tags){
		$this->parse2html('', $custom_tags);
	}
}
?>
